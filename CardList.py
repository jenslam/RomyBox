...
class CardList:
	...
	def readList(self):
		with open(self.path + '/cardList.csv', mode='r') as infile:
			...
			cardList = {rows[0]:rows[2] for rows in reader}
			infile.close()
		return (cardList)
	...
	def getPlaylist(self,card):
		...
		try:
			...
			Logger.logr.error('Card %s is not card list', card)
			return ''

	def getPlaylistName(self,card):
		(self.nameList, self.cardList) = self.readList()
		try:
			return self.nameList[card]
		except:
			Logger.logr.error('Card %s is not card list', card)
			return ''


	def addPlaylist(self, card, plist, pname):
		try:
			if card != ' ':
				if card not in self.cardList.keys():
					f = open(self.path + '/cardList.csv', 'a')
					f.write(card + ',' + pname + "," + plist + '\n')
					self.cardList[card] = plist
					print card + " --> " + pname +" -"  + plist + " added!"
				else:
